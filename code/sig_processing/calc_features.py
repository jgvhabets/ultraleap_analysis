...
def calculate_feat(dist, block, sub, cond, cam, task, side, to_plot = False, to_save_plot = False, to_save = False):
    ...
    """
    Function that calculates uses other functions 
    defined in this .py file to calculate features 
    ...
    idx_max, idx_min = hp.find_min_max(np.array(dist[dist_col]), task=task)
    ...
    ft_dict_tap = get_feat_tap(ls_dist, spe_over_taps, tap_duration)
    ft_dict_block = get_feat_block(ft_dict_tap,
                                   block, sub, cond, cam, task, side
                                   )
...
def get_feat_block(feat_dict, block, sub, cond, cam, task, side):
...
def decrement(feat_dict, max_idx):
